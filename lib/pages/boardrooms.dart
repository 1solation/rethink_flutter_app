...
class Boardroom {
  final String
      chairmanID; // Used to parse the database for the chairman's name (to display).
  final String name; // Boardroom name. Duh.
  final List<String>
      userIDs; // The user IDs are going to be used in order to check if the user is in the boardroom.
  final List<String>
      modIDs; // Used to parse the databse for the moderators names (to display).
  Boardroom(this.chairmanID, this.name, this.userIDs, this.modIDs);
}

class BoardroomsList extends StatelessWidget {
  static List<Boardroom> _getBoardrooms() {
    // Creates the list of boardrooms for the user.
    //TODO: Rewrite function for database authentication logic.

    List<Boardroom> boardroomList = List.generate(
        5,
        (i) => Boardroom('chairman id placeholder',
            'boardroom name placeholder', ["id1", "id2"], ["id3", "id4"]));
          ...
          return Card(
              ...
              child: Column(children: <Widget>[
                ListTile(
                    ...
                    title: Text(boardrooms[index].name,
                    ...
                    onTap: () {
                      Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder: (context) =>
                                BoardroomPage(boardroom: boardrooms[index]),
                          )); // Navigator.push
                    }),
              ]));
        });
  ...
  Widget build(BuildContext context) {
    ...
    return new Scaffold(
        appBar: new AppBar(
          ...
          backgroundColor: Color.fromRGBO(255, 155, 155, 1),
        ),
        body: Container(
